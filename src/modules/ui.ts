...
export class UIManager {
  renderQuestion(
    ...
    const questionText = this.createElement(
      "div",
      "question-text",
      `${questionNumber}. [${question.category}] ${question.question} (Difficulty: ${question.difficulty})`,
      ["my-2"],
      "question-container",
    );
    const answerOptions = this.createElement(
      "div",
      "answer-options",
      "",
      ["row"],
  ...
  showStartScreen(onStart: (playerName: string) => void): void {
  ...
  showGameSummary(gameSummary: GameSummary): void {
    ...
    this.createElement(
      "h2",
      "",
      `Total Score: ${gameSummary.totalScore} %`,
      ["my-2", "text-center"],
      "results",
    );
    this.createElement(
      "h2",
      "",
      `Total Points: ${gameSummary.earnedPoints} / ${gameSummary.maxPoints}`,
      ["my-2", "text-center"],
      "results",
    );

    gameSummary.categoryResults.forEach((result) =>
      this.createElement(
        "div",
        "",
        `${result.category}: ${result.score} % (${result.correctAnswers}/${result.totalQuestions}, ${result.earnedPoints} points)`,
        ["my-2", "text-center"],
        "results",
      ),
    );
    ...
    gameSummary.answerRecords.forEach((result, index) => {
      ...
      this.createElement(
        "div",
        "",
        `Question ${questionNumber}: ${resultText}`,
        ["my-2", "text-center"],
        "results",
      );
    });
  ...
  private createElement<K extends keyof HTMLElementTagNameMap>(
    ...
    if (parentElementId) {
      ...
      if (!parentElement) {
        throw new Error(
          `Parent element with id '${parentElementId}' not found`,
        );
      }
